...
struct ath11k_vif
{
	...
	u32 aid;
	u8 bssid[ETH_ALEN];
	cfg80211_bitrate_mask bitrate_mask;
	int num_legacy_stations;
	int rtscts_prot_mode;
	int txpower;
	bool rsnie_present;
	bool wpaie_present;
	ieee80211_chanctx_conf chanctx;
};
...
{
	ath11k_bp_stats umac_ring_bp_stats[HTT_SW_UMAC_RING_IDX_MAX];
	ath11k_bp_stats lmac_ring_bp_stats[HTT_SW_LMAC_RING_IDX_MAX][MAX_RADIOS];
};
...
struct ath11k_soc_dp_stats
{
	...
	ath11k_soc_dp_tx_err_stats tx_err;
	ath11k_dp_ring_bp_stats bp_stats;
};
...
public:
    ...
    struct ath11k_peer *ath11k_peer_find(struct ath11k_base *ab, int vdev_id, const u8 *addr);
    struct ath11k_peer *ath11k_peer_find_by_addr(struct ath11k_base *ab, const u8 *addr);
    struct ath11k_peer *ath11k_peer_find_by_id(struct ath11k_base *ab, int peer_id);
    int ath11k_core_qmi_firmware_ready(struct ath11k_base *ab);
    int ath11k_core_pre_init(struct ath11k_base *ab);
    int ath11k_core_init(struct ath11k_base *ath11k);
    void ath11k_core_deinit(struct ath11k_base *ath11k);
    struct ath11k_base *ath11k_core_alloc(struct device *dev, size_t priv_size, enum ath11k_bus bus, const struct ath11k_bus_params *bus_params);
