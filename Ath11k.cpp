...
int Ath11k::ath11k_core_suspend(struct ath11k_base *ab)
{
	if (!ab->hw_params.supports_suspend)
  {
      ErrorLog("Suspend not supported\n");
      return false;
  }
	...
	if (ath11k_dp_rx_pktlog_stop(ab, true))
  {
		ath11k_warn(ab, "failed to stop dp rx (and timer) pktlog during suspend: %d\n");
		return false;
	}

	if (ath11k_wow_enable(ab))
  {
		ath11k_warn(ab, "failed to enable wow during suspend: %d\n");
		return false;
	}

	if (ath11k_dp_rx_pktlog_stop(ab, false))
  {
...
int Ath11k::ath11k_core_resume(struct ath11k_base *ab)
{
	...
	if (ath11k_hif_resume(ab))
  {
	...
	if (ath11k_dp_rx_pktlog_start(ab))
  {
		ath11k_warn(ab, "failed to start rx pktlog during resume: %d\n");
		return false;
	}

	if (ath11k_wow_wakeup(ab))
  {
...
void Ath11k::ath11k_core_check_dt(struct ath11k_base *ab)
{
	...
	of_property_read_string(node, "qcom,ath11k-calibration-variant", &variant);
	if (!variant)
  {
      return -ENODATA;
  }
...
void ath11k_core_create_board_name(struct ath11k_base *ab, char *name, size_t name_len)
{
	...
	if (ab->qmi.target.bdf_ext[0] != '\0')
  {
	...
	scnprintf(name, name_len, "bus=%s,qmi-chip-id=%d,qmi-board-id=%d%s", ath11k_bus_str(ab->hif.bus), ab->qmi.target.chip_id, ab->qmi.target.board_id, variant);

	ath11k_dbg(ab, ATH11K_DBG_BOOT, "boot using board name '%s'\n", name);
}

const struct firmware * Ath11k::ath11k_core_firmware_request(struct ath11k_base *ab, const char *file)
{
...
void Ath11k::ath11k_core_free_bdf(struct ath11k_base *ab, struct ath11k_board_data *bd)
{
	if (!IS_ERR(bd->fw))
...
int ath11k_core_parse_bd_ie_board(struct ath11k_base *ab,
...
int ath11k_core_fetch_board_data_api_n(struct ath11k_base *ab, struct ath11k_board_data *bd, const char *boardname)
{
...
void ath11k_core_fetch_board_data_api_1(struct ath11k_base *ab, struct ath11k_board_data *bd)
{
...
int Ath11k::ath11k_core_fetch_bdf(struct ath11k_base *ab, struct ath11k_board_data *bd)
{
...
bool ath11k_core_soc_create(struct ath11k_base *ab)
{
	if (ath11k_qmi_init_service(ab))
  {
		ath11k_err(ab, "failed to initialize qmi :%d\n", false);
		return false;
	}

	if (ath11k_debugfs_soc_create(ab))
  {
...
void ath11k_core_soc_destroy(struct ath11k_base *ab)
{
	ath11k_debugfs_soc_destroy(ab);
	ath11k_dp_free(ab);
	ath11k_reg_free(ab);
...
int ath11k_core_pdev_create(struct ath11k_base *ab)
{
	if (ath11k_debugfs_pdev_create(ab))
  {
		ath11k_err(ab, "failed to create core pdev debugfs: %d\n");
		return false;
	}

	if (ath11k_mac_register(ab))
  {
...
void ath11k_core_pdev_destroy(struct ath11k_base *ab)
{
...
int ath11k_core_start(struct ath11k_base *ab, enum ath11k_firmware_mode mode)
{
	int ret;
...
int Ath11k::ath11k_core_qmi_firmware_ready(struct ath11k_base *ab)
{
	int ret;
...
int ath11k_core_reconfigure_on_crash(struct ath11k_base *ab)
{
	int ret;
...
void Ath11k::ath11k_core_free(struct ath11k_base *ab)
{
	kfree(ab);
}

ath11k_base * Ath11k::ath11k_core_alloc(device *dev, size_t priv_size, enum ath11k_bus bus, const ath11k_bus_params *bus_params)
{
	ath11k_base *ab;
